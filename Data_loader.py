...
for i in range(0, len(tmp_all_images)):
    if i % 2 == 0:
        # all_images.append([np.concatenate(tmp_all_imageCOs[i][0], tmp_all_images[i+1][0]),tmp_all_images[i][0]])
        ...
        all_images.append(np.concatenate(tmp_all_images[i][0], tmp_all_images[i+1][0],zeros(128,128),))
    else:
        continue
